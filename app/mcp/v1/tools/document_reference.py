...
async def request_document_reference_resource(
    ...
    Rules:
        - When creating or updating a document reference, use only the data explicitly
          provided by the user.
        - Do not guess, auto-fill, or assume any missing data.
        - When deleting a document reference, ask the user for confirmation with details
          of the document and wait for the user's confirmation.
...
async def add_document_to_pinecone(document: Document) -> str | PineconeError:
    """
    IMPORTANT: Always inform the user at the beginning of your response that this search
    operation may take some time because the embedding model will be loaded into cache.
    ...
    Rules:
        - If you cannot determine the format of the document fitting the format from the
          list, provide the format as None.
        - After adding the document, the Pinecone index may take up to 1 minute to update
          before the document is searchable.

    Args:
        url (str): The URL of the document to be added.
        fhir_document_id (str): The ID of the FHIR DocumentReference resource
...
async def search_pinecone(
    ...
    IMPORTANT: Always inform the user at the beginning of your response that this search
    operation may take some time because the embedding model will be loaded into cache.
    Searches the Pinecone vector index for information related to the given document by
    ...
    Rules:
        ...
        - If the error message "Document does not exist in Pinecone index" is returned,
          automatically trigger the 'add_document_to_pinecone' tool to add the missing
          document to the index.
        - Translate the user's query into the language of the document before performing
          the search.
        - Base all answers strictly on the content found in the Pinecone index documents.
        - If the user's question is unrelated to the indexed documents, respond that the
          information is not available in the documents.
        - If the query is unclear or ambiguous, ask the user to clarify or provide more
          details.
        - You can modify the query to make it more specific and relevant to the document.

    Args:
        query (str): The user's search query.
        fhir_document_id (str): The ID of the FHIR DocumentReference resource to search
            within - it is the same as the FHIR ID of the document.
